...
# print("\n".join(sys.path))
import autoLayerTools.command as cmd
from autoLayerTools.scripts.core import upEnv
from autoLayerTools.ui.resources import optTxtEdt
import autoLayerTools.ui.resources.execTxtEdt;reload(autoLayerTools.ui.resources.execTxtEdt)
...
class BnStandaloneTools_UI(QMainWindow):
    ...
    def setupUi2(self):
        ...
        self.exec_txt = autoLayerTools.ui.resources.execTxtEdt.ExecTxtEdt(self.ui.frm_exec)
        ...
        for e_bt in self.pb_lst:
            ...
            bt_nm_strip = re.sub("\d$", "", e_bt.objectName())
            if not bt_nm_strip in ['pb_run', 'pb_browse']:
                _fuction = getattr(self, "_cmd_{}".format(bt_nm_strip)) if "_cmd_{}".format(
                    bt_nm_strip) in self.__class__.__dict__ else lambda: self.someFunc(e_bt)
                e_bt.clicked.connect(_fuction)

        self.ui.pb_run.clicked.connect(self.runIt)
        self.ui.pb_browse.clicked.connect(lambda q='search': self._cmd_pb_browse(q))
        self.ui.pb_browse1.clicked.connect(lambda q='out': self._cmd_pb_browse(q))
        ...
        self.ui.chbx_sub_opt.setVisible(0)
        self.ui.chbx_sub_srch.setVisible(0)
    @Slot()
    ...
    def reset(self):
        ...
        self._maya_location = None
        self._python_home = None
    def _cmd_pb_other(self):
        print("................me.....em...........")

    def fn_obtain_all_necessity(self):
        self._searchDir = self.ui.le_search.text()
        self._outputDir = self.ui.le_opt.text()
        if not self._searchDir:
            logger.error("You need to assign a directory that contains animation file ")
    def runIt(self):
        print("...............em....????em????????")
        if not self._mayapy_path:
            logger.error("DO NOT FIND mayapy.exe !!!!!")
            return
        logger.info(">>>Output Director : < {} >".format(self._outputDir))
        logger.info("Ready to ...............")
        self.fn_obtain_all_necessity()
        self.run_autoLayer()

    def run_autoLayer(self):
        mayapy = self._mayapy_path
        code_path = self._exec
        andir = self._searchDir
    ...
    def _invoke_mayapy(self):
        ...
        if str(exitcode) != '0':
            ...
            print(">>>Successful!!!!")
    def _cmd_pb_browse(self,q):
        dialog = QFileDialog()
        foo_dir = dialog.getExistingDirectory(self, 'Select an awesome directory')
        if q == 'search': self.ui.le_search.setText(foo_dir)
        elif q == 'out': self.ui.le_opt.setText(foo_dir)
    ...
    def fn_set_autlayer2defualt_exec(self):
        ...
        :return:
        """
        studioDir = cmd.tierpath(sys.argv[0],2)
        autoLayer_py = cmd.joinpath(studioDir,'scripts','execs','anAutoLayer.py')
